...
(function initCursor() {
  ...
  hoverEls.forEach(el => {
    el.addEventListener('mouseenter', () => {
      ...
      cursor.style.background = 'rgba(0,229,176,0.5)';
  ...
  pts.forEach((a, i) => {
    pts.forEach((b, j) => {
      ...
      if (d < 140) {
        ...
        line.setAttribute('stroke', 'rgba(0,229,176,0.25)');
...
(function initCardGlow() {
  document.querySelectorAll('.feature-card, .testi-card, .price-card').forEach(card => {
    card.addEventListener('mousemove', e => {
      ...
      card.style.background = `radial-gradient(circle 180px at ${x}px ${y}px, rgba(0,229,176,0.05), var(--surface) 70%)`;
    });
...
console.log('%cAxialNet — Network Online ✓', 'color:#00e5b0;font-family:monospace;font-size:14px;font-weight:bold;');
